...
{
    public partial class ManagerSuppliersForm : Form
    {
        public ManagerSuppliersForm()
        {
            InitializeComponent();
        }
        ...
        {
            AddEditSupplierForm addSupplierForm = new AddEditSupplierForm();
            this.Hide();
            addSupplierForm.Show();
        }

        private void ManagerSuppliersForm_Load(object sender, EventArgs e)
        {
            SqlConnection connection = SessionState.GetConnection();
            SqlCommand command = new SqlCommand();
            connection.Open();
            command.Connection = connection;
            command.CommandText = "SELECT * FROM [cafe_Management_system].[dbo].[Suppliers]";
